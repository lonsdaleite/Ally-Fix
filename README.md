...
| **Vibration Fix** | Sets grip-motor vibration intensity to **50 %** per motor. Scale 0–100, firmware default 100/100. Written through the kernel's `vibration_intensity` sysfs attribute; that store handler currently rejects values above 64, in which case the plugin writes 64 there and sends the real value to the controller itself (same MCU command, feature report on the config interface). Re-applied when the controller re-enumerates (sleep/resume, InputPlumber restart). Settings: link/unlink motors, per-motor sliders (reset to 50/50 whenever the fix is enabled), test rumble. Grip motors only. The card also hosts an **Enhanced Vibration** toggle — the "Xbox-recommended waveform" option from Armoury Crate, sent as a single MCU command. The flag is stored inside the controller and survives reboots and OS switches; the toggle is independent of the fix, untouched by *Fix all*, available on the Xbox Ally / Ally X only, and turned off when the plugin is uninstalled. Heads-up: with it on, full-strength rumble from games sounds like a rattle on SteamOS (the kernel driver sends out-of-range magnitudes). |
...
reverts the fixes, switches Enhanced Vibration off and deletes the InputPlumber override (the
last two are the changes that survive a reboot):
...
- Uninstalling the plugin through Decky reverts every enabled fix (boost on, vibration 100/100, fan
  curve back to auto, InputPlumber override removed, Enhanced Vibration off). This runs inside the plugin process, so it
